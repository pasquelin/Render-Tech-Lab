import { chromium, BASE_FLAGS, explorerOptions, sdkUrl, manifestUrlFor, URL_BASE } from './lib.mjs';
const engines = process.argv.slice(2).length ? process.argv.slice(2) : ['exact-cluster-pages','webgpu-page-raster'];
const browser = await chromium.launch({ headless: true, executablePath: '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome', args: [...BASE_FLAGS, '--enable-features=Vulkan,WebGPU'] });
for (const engineId of engines) {
  ...
  }, { sdkUrl, engineId, options: explorerOptions(manifestUrlFor('emerald-square'), { clearColor: undefined }) });
