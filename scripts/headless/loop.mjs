...
const r = await page.evaluate(async (o) => {
  ...
  let explorer;
  try {
    explorer = await createExplorer(canvas, { ...o.options, backends: [benchEngine(o.engineId).factory], onDiagnostic: d => { if (diags.length < 60 && /fail|error|capacity|coverage|fallback|incomplete/i.test(d.phase + ' ' + d.message)) diags.push({ phase: d.phase, message: d.message, context: JSON.stringify(d.context ?? {}).slice(0, 300) }); } });
...
}, { sdkUrl, engineId, options: explorerOptions(manifestUrlFor(process.env.SCENE ?? 'emerald-square'), { pixelError, preload, diagnosticDetail: detail, replicaCount: Number(process.env.REPLICAS ?? 1) }) });
