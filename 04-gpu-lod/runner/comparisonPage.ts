...
let comparisonMounted = false;

export function mountComparisonPage(): () => void {
if (comparisonMounted) return () => undefined;
comparisonMounted = true;
const listeners = new AbortController();
const on = (node: EventTarget, event: string, handler: EventListener) =>
  node.addEventListener(event, handler, { signal: listeners.signal });
...
on(stopButton, 'click', () => { stop(); setStatus('Scène arrêtée.'); });
on(previewButton, 'click', async () => {
