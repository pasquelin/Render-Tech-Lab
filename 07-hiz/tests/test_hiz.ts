...
export function runHiZSuite() {
  ...
  const reversed = buildHiZPyramid([[0.1, 0.8, 0.4]], true);
  assert(reversed.pyramid.mips.at(-1)?.width === 1, 'Une ligne impaire doit terminer en 1x1');
  assert(Math.abs(reversed.mipBuffers.at(-1)![0] - 0.1) < 1e-6, 'Reversed-Z doit conserver le minimum');
  assert(queryHiZ(reversed, { x0: 0, y0: 0, x1: 2, y1: 0 }, 0.05, true).occluded, 'Reversed-Z doit rejeter un candidat plus lointain');

  let invalidQueryRejected = false;
  try { queryHiZ(pyramidData, { x0: 4, y0: 2, x1: 1, y1: 3 }, 0.5); } catch { invalidQueryRejected = true; }
  assert(invalidQueryRejected, 'Une empreinte inversée doit être rejetée explicitement');
