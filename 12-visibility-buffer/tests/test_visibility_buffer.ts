/**
 * 12-visibility-buffer/tests/test_visibility_buffer.ts
 *
...
export function runVisibilityBufferSuite() {
  ...
  for (const [inst, prim] of testPairs) {
    ...
    const unpacked = unpackVisibilityId(packed);
    if (unpacked === null) throw new Error('Un ID de géométrie ne doit pas être interprété comme fond');
    const [unpackedInst, unpackedPrim] = unpacked;
    assert(
      unpackedInst === inst && unpackedPrim === prim,
      `Échec roundtrip visibilityId pour (${inst}, ${prim}) -> ${packed} -> (${unpackedInst}, ${unpackedPrim})`
    );
  }
  assertThrowsCapacity();
...
function assertThrowsCapacity() {
  let rejected = false;
  try { packVisibilityId(65535, 65535); } catch { rejected = true; }
  assert(rejected, 'La paire maximale doit être refusée car zéro est réservé au fond');
}
