...
**Statut :** reconstruction, correctness gate et orchestration bornée implémentés. La campagne WebGPU reste `not-run` jusqu'au branchement d'un exécuteur matériel dans `bench/main.ts`.
...
`runVisibilityCampaign` reçoit les paliers, warmup, échantillons, `AbortSignal`, progression et un exécuteur. L'exécuteur renvoie les readbacks attendus/réels et les temps mesurés séparément. Une différence d'ID ou de profondeur invalide immédiatement la campagne ; aucun chiffre de performance n'est alors publiable.
