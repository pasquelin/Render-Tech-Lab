# Rapport de banc — 12-visibility-buffer

> **`Visibility buffer & shading différé`**
...
- `12-visibility-buffer/types.ts` — contrats de type (squelette, aucun algorithme)
- `12-visibility-buffer/benchmark.md` — cahier des charges (paliers, métriques obligatoires, règle d'arbitrage)
- `12-visibility-buffer/implementation.md` — plan d'implémentation (non livré)
- `12-visibility-buffer/results/latest.json` — squelette `not-run` (champs `null` conformément au schéma)
