# Rapport de banc — 11-geometry-streaming

> **`Résidence VRAM & pression mémoire (10% → 100%)`**
...
- `11-geometry-streaming/types.ts` — contrats de type (squelette, aucun algorithme)
- `11-geometry-streaming/benchmark.md` — cahier des charges (paliers, métriques obligatoires, règle d'arbitrage)
- `11-geometry-streaming/implementation.md` — plan d'implémentation (non livré)
- `11-geometry-streaming/results/latest.json` — squelette `not-run` (champs `null` conformément au schéma)
