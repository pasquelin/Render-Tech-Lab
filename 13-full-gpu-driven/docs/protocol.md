...
Ce banc n’instancie pas encore `shared/scene` : `test_full_pipeline.ts` reste un oracle d’assemblage (`blocked` tant que les dépendances ne sont pas `measured` / `INTEGRATE`). Paliers visés, non mesurés :
