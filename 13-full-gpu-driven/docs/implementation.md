...
**Statut :** orchestrateur de préparation implémenté ; aucun pipeline physique ni benchmark réel n'a été exécuté. Le banc reste `not-run` tant que ses dépendances et son assemblage ne sont pas prêts.
...
## 2. Architecture et gate actuellement exécutables
...
`inspectPipelineReadiness()` contrôle chaque dépendance 01→12. Une absence ou un statut `not-run`, un verdict indécis et un rejet sont rapportés séparément. `executeFullPipeline()` ne fabrique aucune métrique : il renvoie `blocked` tant que ces gates échouent et refuse l'exécution lorsque les gates passent mais que le pipeline physique n'est pas encore branché.

Aucune optimisation n'est introduite ici : le banc 13 **mesure** la somme des stages et le compare à 00-baseline.

## 3. Métriques consolidées (voir `13-full-gpu-driven/contracts.ts`)
