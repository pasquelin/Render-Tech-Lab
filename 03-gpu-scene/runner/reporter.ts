import type { GpuSceneBenchResult, SceneStressConfig } from '../contracts.ts';
...
function scenarioKey(c: SceneStressConfig): string {
  return `${c.dimension}|${c.name}|${c.objectCount}|${c.geometryCount}|${c.materialCount}|${c.dynamicRatio}|${c.targetVisibility}`;
}
...
export function formatGpuSceneReport(
  ...
  const now = timestamp.replace('T', ' ').slice(0, 19);
