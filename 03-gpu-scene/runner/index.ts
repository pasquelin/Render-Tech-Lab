...
import { generateStressScene } from '../scenarios/stressScenarios.ts';
import { ClassicMultiMeshScene } from '../implementation/classicMultiMeshScene.ts';
import { GPUSceneRenderer } from '../implementation/gpuSceneRenderer.ts';
import type { SceneStressConfig, GpuSceneBenchResult } from '../contracts.ts';
...
export class GPUSceneBenchmarkRunner {
  ...
  public onModeChange?: (mode: 'classic' | 'gpu-scene') => void;
  ...
  private renderFrame(time: number, timer?: TimestampBatch, sample = 0): FrameSample {
    ...
    } else if (this.currentMode === 'gpu-scene' && this.gpuSceneRenderer) {
  ...
  public async runCampaign(
    ...
    try {
      for (let sIdx = 0; sIdx < scenarios.length; sIdx++) {
        ...
        if (this.onResult) this.onResult(results.at(-1)!);
