...
export function IntegrationFixtureLab({onScene}:{onScene:(scene:IntegrationScene)=>void}){
 ...
 const openReport=async()=>{setState(old=>({...old,reportModal:{...old.reportModal,open:true,title:'Rapport · Fixture procédurale',path:'reports/15-virtualized-integration.md & 15-virtualized-integration/results/REPORT.md',html:'<p>Chargement du rapport…</p>',feedback:''}}));try{const report=await loadMarkdownReport(fetch,test);setState(old=>({...old,reportModal:{...old.reportModal,raw:report.ok?report.raw:'',html:report.ok?parseMarkdownToHtml(report.raw):`<p>${report.raw}</p>`,feedback:report.feedback}}));}catch{setState(old=>({...old,reportModal:{...old.reportModal,html:'<p>Rapport indisponible. Réessayez.</p>'}}));}};
 const copyReport=()=>{if(!state.reportModal.raw)return;void navigator.clipboard.writeText(state.reportModal.raw).then(()=>setState(old=>({...old,reportModal:{...old.reportModal,feedback:'Markdown copié dans le presse-papier.'}}))).catch(()=>setState(old=>({...old,reportModal:{...old.reportModal,feedback:'Copie indisponible.'}})));};
 const openFinder=()=>{void fetch('/api/open-folder',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({testId:test,folder:'reports'})}).then(response=>response.json()).then(data=>setState(old=>({...old,reportModal:{...old.reportModal,feedback:`Finder ouvert : ${data.targetFile??data.targetDir??'reports/'}`}}))).catch(()=>setState(old=>({...old,reportModal:{...old.reportModal,feedback:'Rapports : ./reports/'}})));};
