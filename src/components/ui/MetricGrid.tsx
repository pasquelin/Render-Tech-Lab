export type MetricData = { id?: string; label: string; value: string; unit?: string; provenance?: string; status?: string; ratio?: number };
  ...
  return (
    <section aria-label={label} className="space-y-2">
      ...
      <dl className="grid grid-cols-2 gap-3 text-xs">
        {items.map(item => (
          <div key={item.id ?? item.label} className="rounded-box bg-base-300/60 p-3">
            <dt className="text-base-content/60">{item.label}</dt>
            <dd id={item.id} className="font-mono break-words">{item.value}{item.unit ? ` ${item.unit}` : ''}</dd>
