/** Une ligne : autant de cellules que de colonnes, plus une provenance optionnelle sous la première,
 *  comme la provenance d'une carte de `MetricGrid`. */
export type MetricTableRow = { id?: string; cells: readonly string[]; provenance?: string };
 ...
 * tableaux des rapports. `provenance` est celle du tableau entier, sous le tableau.
 */
            ...
            {rows.map((row, rowIndex) => (
              <tr key={row.id ?? rowIndex} id={row.id}>
                {row.cells.map((cell, cellIndex) => (
                  <td key={cellIndex} className={'align-top ' + (cellIndex === 0 ? 'break-words' : 'whitespace-nowrap')}>
                    {cell}
                    {cellIndex === 0 && row.provenance ? <p className="text-[9px] text-base-content/45 whitespace-normal">{row.provenance}</p> : null}
                  </td>
                ))}
              </tr>
            ))}
