...
Chaque valeur de matériau peut propager `(value,dx,dy)`. Somme, produit, quotient et normalisation suivent leurs jacobiennes ; voir [les dérivées](SCHEMAS_ET_DERIVEES.md). Une discontinuité de `fract`, `step` ou d'une branche demande un filtrage explicite, pas une dérivée arbitrairement nulle.
...
Cette statistique ne définit ni occupancy, ni densité, ni shader volumétrique, ni format de streaming. Pour un futur banc voxel, ajouter grille/hiérarchie, règle de couverture, matériaux, intégration d'opacité et erreur de silhouette. Garder le rendu triangulé comme référence.
