...
Définir un ordre total du gagnant : profondeur d’abord, identité stable ensuite pour les égalités. L’identité ne doit jamais déborder sur les bits qui ordonnent la profondeur. Une représentation atomique large peut publier la paire indivisiblement sur un backend compatible ; la référence portable utilise une cible d’identité sous le même depth test matériel.
...
Un facteur de subdivision par arête peut être proposé à partir de sa longueur projetée ℓ et d’un objectif s>0 : n=max(1,ceil(ℓ/s)). Ce critère borne une taille d’arête, pas l’erreur du déplacement. Calculer le même n pour les deux côtés d’une arête partagée, puis utiliser un motif conforme. Lorsque les facteurs dépassent la capacité locale d’un motif, subdiviser d’abord en sous-patches, puis appliquer les motifs bornés.
