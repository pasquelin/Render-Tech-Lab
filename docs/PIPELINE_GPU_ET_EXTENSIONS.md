...
Référence déterministe, volontairement coûteuse : traiter les primitives par ID global stable croissant, avec un dessin matériel séparé et ordonné par primitive, et un test de profondeur strict (LESS en standard, GREATER en inversé). À profondeur égale, le plus petit ID déjà écrit gagne. Le chemin logiciel applique le même ordre profondeur/ID. Chaque chemin produit sa paire gagnante et la composition applique encore cet ordre. Les clés comprennent l’instance ; le fond a un état invalide distinct.
