...
La page est une unité de stockage/résidence, pas automatiquement une région LOD. Une représentation peut demander plusieurs pages ; toutes doivent être utilisables avant son activation. Les détails de format et publication sont dans [pages et mémoire](PAGES_MEMOIRE_ET_CACHE.md).
