...
import {
  MAX_SHADOWED_LIGHTS_PER_FRAME, BOUNCE_DIAGNOSTIC, DEFAULT_LIGHTING_BACKEND, DEFAULT_LIGHTING_BOUNCE, DEFAULT_LIGHTING_CAMERA, defaultConfig, emptyLightingStats, sunLights,
...
export async function createLightingBench(
  ...
  const explorer = await createExplorer(canvas, {
    ...
    bounce: options.bounce ?? DEFAULT_LIGHTING_BOUNCE,
    onDiagnostic: diagnostic => {
      if (diagnostic.phase !== BOUNCE_DIAGNOSTIC) return;
      const reason = diagnostic.context.unavailable;
      bounceNotice = typeof reason === 'string' ? reason : null;
    },
