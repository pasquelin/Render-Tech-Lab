...
try{
  ...
  assert.deepEqual(sections,['lab-run-card','lab-mode-card','lab-metrics-card','lab-report-card']);
  ...
  await page.selectOption('#lighting-run-kind','comparison');await page.click('#btn-benchmark');
  console.log(JSON.stringify({phase:'campaign-started',out}));
  await page.waitForFunction(()=>!!window.__lightingBench16Report||document.querySelector('[data-execution-view="error"]'),null,{timeout:360000});
  const failure=await page.locator('[data-execution-view="error"]').count();
  if(failure)throw Error(await page.locator('[data-execution-view="error"]').innerText());
