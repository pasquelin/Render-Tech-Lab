 ...
 *
 * Le partitionneur CPU de référence est fourni dans implementation/. Il sert
 * d'oracle de correction; il ne constitue pas une optimisation ni une mesure GPU.
 */
...
export interface NormalCone {
  ...
  cosHalfAngle: number;
  /** Faux lorsque les normales ne permettent pas un rejet conservateur. */
  cullable: boolean;
}
...
export interface Meshlet {
  ...
  /** Indices globaux réellement référencés par ce cluster, sans supposer leur contiguïté. */
  vertexIndices: Uint32Array;
  /** Premier triangle source; permet de certifier la couverture sans trou ni doublon. */
