...
- Consommation des fixtures `shared/fixtures/stress.ts` et `createTriangleMesh` comme invariants géométriques.
- Consommation du runner canonique `shared/benchmark/runner.ts` pour enregistrer `not-run` / `measured` sans inventer.
- Consommation des types `05-meshlets/types.ts` comme contrat de métadonnées partagées (05, 06, 07, 13).
...
| Livrable | État |
|---|---|
| `05-meshlets/types.ts` | ✅ Présent |
| `05-meshlets/benchmark.md` (contrat) | ✅ Présent |
| `05-meshlets/benchmark/runner.ts` | ❌ À créer lors de l'implémentation |
