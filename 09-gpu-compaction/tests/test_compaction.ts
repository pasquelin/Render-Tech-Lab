/**
 * 09-gpu-compaction/tests/test_compaction.ts
 *
...
export function runCompactionSuite() {
  ...
  for (const n of scales) {
    ...
    for (const v of variants) {
      ...
      assert(output.execution === 'cpu-reference', `${v} doit annoncer explicitement une exécution CPU`);
      ...
      if (v !== 'one-thread-per-command') {
        ...
        assert(
          output.compactedIndices.every((index, position) => index === position * 2),
          `La liste compactée ${v} doit préserver exactement les indices visibles`
        );
      }

      console.log(
        `    - [${v.padEnd(23)}] : Oracle CPU = ${output.timeMs?.toFixed(2)} ms | Opérations atomiques logiques = ${output.atomicOperations} | Compactés = ${output.compactedCount}`
      );
      ...
      results.push({
        ...
        atomicOperations: output.atomicOperations ?? 0,
      });
    }
  }
  ...
  assert(
    atomic1M.atomicOperations === atomic1M.compactedCount,
    'atomicAdd doit effectuer une opération atomique logique par élément visible'
  );
  assert(
    scan1M.atomicOperations === 0,
    'parallel-prefix-scan ne doit effectuer aucune opération atomique globale'
  );
