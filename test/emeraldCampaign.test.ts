...
test('summaries contain actual percentiles and history retains five unique runs',()=>{assert.equal(distribution([]),null);assert.deepEqual(distribution([1,2,3,4,5,NaN]),{count:5,p50:3,p95:5,p99:5,max:5});const runs=Array.from({length:6},(_,i)=>({id:String(i)}));assert.deepEqual(retainReports(runs as never,{id:'2'} as never).map(r=>r.id),['2','0','1','3','4']);});
function report(engine:EmeraldReport['configuration']['engine'],mutator?:(value:EmeraldReport)=>void):EmeraldReport{
 ...
 const value:EmeraldReport={version:1,id:engine,timestamp:'2026-09-12T00:00:00.000Z',status:'completed',configuration:{cities:1,detail:'source',lodQuality:'high',mode:'path',camera:'orbit',diagnostic:'beauty',layout:'single',engine,compareEngine:'three-webgl-reference',poi:null,wipe:.5},pathEngines:[engine],sourceKey:'k',availableTriangles:1,sharedGeometry:true,multipliedInstances:1,resolution:[640,360],firstImageMs:1,preparationMs:1,warmupFrames:30,samples,captures:[],error:null,fallbacks:[],retainedSamplesOnly:false,environment:'test',pathVersion,comparison:'visual-only',comparisonReason:'blocked'};
...
test('a path campaign still records engine, pose and counters for each photo',()=>{
 ...
 assert.equal(still.name,'Vue générale de la ville');
