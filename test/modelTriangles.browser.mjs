...
try{
 ...
 await mkdir('benchmark-runs/checks/model-triangles',{recursive:true});
 ...
 const canvas=page.locator('#canvas-model');
 const triangles=await canvas.screenshot({path:'benchmark-runs/checks/model-triangles/triangles.png'});
 ...
 const clusters=await canvas.screenshot({path:'benchmark-runs/checks/model-triangles/clusters.png'});
 ...
 await page.waitForTimeout(400);
 const beauty=await canvas.screenshot({path:'benchmark-runs/checks/model-triangles/beauty.png'});
 assert.notDeepEqual(triangles,beauty);
 const report=await page.evaluate(()=>JSON.parse(localStorage.getItem('render-tech-lab:model-runs:v1')??'[]'));
 await page.getByRole('button',{name:'Arrêter l’exploration'}).click();
 const last=await page.evaluate(()=>JSON.parse(localStorage.getItem('render-tech-lab:model-runs:v1'))[0]);
