...
import { checkEmeraldAvailability } from '../src/lab/emeraldAvailability.ts';
...
test('missing cache gives an actionable failure without claiming preparation is active', async()=> {
 await assert.rejects(checkEmeraldAvailability((async()=>new Response('{}',{status:404})) as typeof fetch),/HTTP 404.*Préparez/);
});
