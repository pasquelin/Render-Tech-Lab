...
export interface WorldCapture {
  ...
  maxRepeatChannelDifference?: number; maxCandidateChannelDifference?: number;
...
export interface WorldReport {
  ...
  quality: {
    passed: boolean; captures: WorldCapture[]; failure?: string; checkedFrames: number[];
    selectionOracle?: { sampleCount: number; mismatches: number; checked: boolean };
  };
  blocks: WorldBlock[]; records: BenchResultRecord[]; limitations: string[];
  preparationMs: number; qualityMs: number;
}
export type WorldProgressPhase = 'asset-download' | 'asset-decode' | 'scene-preparation' | 'quality-control' |
  'first-frame' | 'common-warmup' | 'block-warmup' | 'measurement' | 'gpu-drain' | 'complete' | 'cancelled';
export interface WorldProgress {
  phase: WorldProgressPhase; message: string; completed?: number; total?: number;
  unit?: 'bytes' | 'districts' | 'frames' | 'captures' | 'blocks'; assetBytes?: number;
}
export interface WorldPreviewCompletion { reason: 'duration' | 'manual' | 'cancelled' | 'error'; elapsedMs: number; error?: Error }
