...
import {
  adaptiveVisible, centerLengthOf, prepareCoherence, resetAdaptiveEntry, type CoherenceState,
} from './adaptiveCulling.ts';
import type { WorldOptions, WorldMode, WorldFrame, WorldBlock, WorldReport, WorldCapture, WorldSummary, WorldPreviewMetrics, WorldProgress, WorldPreviewCompletion } from '../contracts.ts';
export type { WorldOptions, WorldReport, WorldPreviewMetrics, WorldProgress, WorldPreviewCompletion } from '../contracts.ts';
const ASSET = '/benchmark-assets/bistro/bistro-exterior.glb';
const nowISO = () => new Date().toISOString();
const nextFrame = () => new Promise<number>(resolve => requestAnimationFrame(resolve));
export function createPreviewClock(durationMs:number){
  if(!Number.isFinite(durationMs)||durationMs<=0)throw new Error('Durée de prévisualisation invalide.');
  let startedAt:number|null=null;
  return {sample(time:number){
    if(!Number.isFinite(time))throw new Error('Horodatage de prévisualisation invalide.');
    startedAt??=time;const elapsedMs=Math.min(durationMs,Math.max(0,time-startedAt));
    return {elapsedMs,fraction:elapsedMs/durationMs,completed:elapsedMs>=durationMs};
  }};
}
export function fitPreviewViewport(containerWidth:number,containerHeight:number,fallbackWidth:number,fallbackHeight:number){
  const width=containerWidth>0?containerWidth:fallbackWidth,height=containerHeight>0?containerHeight:fallbackHeight;
  if(![width,height].every(value=>Number.isFinite(value)&&value>0))throw new Error('Dimensions de prévisualisation invalides.');
  const cssWidth=Math.max(1,Math.round(width)),cssHeight=Math.max(1,Math.round(height));
  return {cssWidth,cssHeight,drawingWidth:cssWidth,drawingHeight:cssHeight};
}
export function createWorldProgressEmitter(legacy:(message:string)=>void,onLifecycle?:(progress:WorldProgress)=>void){
  return (state:WorldProgress)=>{legacy(state.message);onLifecycle?.(state);};
}
...
async function execute(canvas: HTMLCanvasElement, options: WorldOptions, progress: (message: string)=>void,
  ...
  onMetrics?: (metrics: WorldPreviewMetrics)=>void, onLifecycle?: (progress:WorldProgress)=>void,
  onPreviewComplete?: (completion:WorldPreviewCompletion)=>void): Promise<WorldReport | (()=>void)> {
  ...
  let disposed=false, ownsPreview=false, request=0, hidden=document.visibilityState!=='visible',resizeObserver:ResizeObserver|null=null;
  let previewElapsedMs=0,previewCompletionSent=false;
  const visibility=()=>{if(document.visibilityState!=='visible')hidden=true;};
  document.addEventListener('visibilitychange',visibility);
  const emit=createWorldProgressEmitter(progress,onLifecycle);
  const completePreview=(reason:WorldPreviewCompletion['reason'],error?:Error)=>{
    if(previewCompletionSent)return;previewCompletionSent=true;onPreviewComplete?.({reason,elapsedMs:previewElapsedMs,...(error?{error}:{})});
  };
  const cleanup=()=>{
    ...
    textures.forEach(t=>{const data:unknown=t.source.data;
      ...
      t.dispose();});bitmaps.forEach(image=>image.close());
    renderer.dispose();
  };
  const cancel=()=>{completePreview(signal?.aborted?'cancelled':'manual');cleanup();};
  ...
  try {
    ...
    canvas.style.position='absolute';canvas.style.inset='0';canvas.style.width='100%';canvas.style.height='100%';canvas.style.display='block';canvas.style.objectFit=preview?'fill':'cover';
    ...
    emit({phase:'asset-download',message:'Chargement du décor Bistro et de ses textures originales.',completed:0,unit:'bytes'});
    const response=await fetch(ASSET,{signal});if(!response.ok)throw new Error(`Chargement Bistro : HTTP ${response.status}.`);
    const buffer=await response.arrayBuffer();emit({phase:'asset-download',message:`Décor Bistro chargé : ${buffer.byteLength.toLocaleString('fr-FR')} octets.`,completed:buffer.byteLength,total:buffer.byteLength,unit:'bytes',assetBytes:buffer.byteLength});
    check();const assetHash=await hash(buffer);check();emit({phase:'asset-decode',message:'Décodage du GLB et de ses textures.',completed:0});
    ...
    for(let i=0;i<config.districts;i++){
      ...
      if(i%3===2||i===config.districts-1){emit({phase:'scene-preparation',message:`Décor résident : ${i+1}/${config.districts} quartiers.`,completed:i+1,total:config.districts,unit:'districts'});await nextFrame();check();}
    }
    ...
    const resizePreview=()=>{if(!preview||disposed)return;const host=canvas.parentElement;
      const viewport=fitPreviewViewport(host?.clientWidth??canvas.clientWidth,host?.clientHeight??canvas.clientHeight,config.width/config.pixelRatio,config.height/config.pixelRatio);
      renderer.setPixelRatio(1);renderer.setSize(viewport.drawingWidth,viewport.drawingHeight,false);camera.aspect=viewport.drawingWidth/viewport.drawingHeight;camera.updateProjectionMatrix();};
    if(preview){resizePreview();if(typeof ResizeObserver!=='undefined'){resizeObserver=new ResizeObserver(resizePreview);if(canvas.parentElement)resizeObserver.observe(canvas.parentElement);}}
    ...
    const sphereEntries=meshes.map(mesh=>{
      ...
      return {mesh,sphere,center:sphere.center,radius:sphere.radius,centerLength:centerLengthOf(sphere.center),lastPlane:0,margin:-Infinity,mask:mesh.layers.mask,test:mesh.layers.test};
    });
    ...
    const isAdaptive=(value:WorldMode|null):value is 'adaptive-frustum'|'adaptive-coherent'=>value==='adaptive-frustum'||value==='adaptive-coherent';
    const coherence:CoherenceState={previousPlanes:new Float64Array(24),havePreviousPlanes:false};
    ...
    const cull=()=>{activeDistricts=0;adaptivePlaneTests=adaptiveRejectedMeshes=adaptiveCertifiedMeshes=0;
      if(mode==='adaptive-coherent'){const step=prepareCoherence(frustum.planes,coherence);deltaNormal=step.deltaNormal;deltaConstant=step.deltaConstant;constantScale=step.constantScale;}
      ...
      if(isAdaptive(mode))for(const entry of sphereEntries){
        const decision=adaptiveVisible(entry,frustum.planes,mode,deltaNormal,deltaConstant,constantScale);
        entry.mesh.layers.mask=decision.visible?entry.mask:0;
        adaptivePlaneTests+=decision.planeTests;
        if(decision.certified)adaptiveCertifiedMeshes++;
        if(!decision.visible)adaptiveRejectedMeshes++;
      }
    };
    ...
    if(preview){
      let previous:number|null=null;const clock=createPreviewClock(config.previewDurationSeconds*1000);
      const tick=(time:number)=>{if(disposed)return;if(document.visibilityState!=='visible'){previous=null;request=requestAnimationFrame(tick);return;}
        try{check();const begin=performance.now();const state=clock.sample(time);previewElapsedMs=state.elapsedMs;
          ...
          previous=time;if(state.completed){completePreview('duration');cleanup();return;}request=requestAnimationFrame(tick);
        }catch(error){const failure=error instanceof Error?error:new Error(String(error));completePreview(signal?.aborted?'cancelled':'error',failure);cleanup();onPreviewError?.(failure);}};
      signal?.throwIfAborted();ownsPreview=true;signal?.addEventListener('abort',cancel,{once:true});request=requestAnimationFrame(tick);return cancel;
    }
    ...
    emit({phase:'common-warmup',message:`Échauffement commun avant contrôle des images : ${config.warmup} images.`,completed:0,total:config.warmup,unit:'frames'});
    for(let i=0;i<config.warmup;i++){await nextFrame();check();pose(i*(config.samples-1)/Math.max(1,config.warmup-1));cull();render();if(i===0)emit({phase:'first-frame',message:'Première frame présentée.',completed:1,total:1,unit:'frames'});if(i%15===14||i===config.warmup-1)emit({phase:'common-warmup',message:`Échauffement commun : ${i+1}/${config.warmup} images.`,completed:i+1,total:config.warmup,unit:'frames'});}
    ...
    const selectionOracle={sampleCount:0,mismatches:0,checked:false};
    const limitations=[
    ...
    const report=(passed:boolean,failure?:string):WorldReport=>{
      ...
      if(!passed)records.push({timestamp,test:'14-open-world:quality',commit:null,status:'measured',verdict:'REJECT',environment:{browser:navigator.userAgent,threeVersion:THREE.REVISION},scene:{triangles:sceneInfo.sourceTriangles},cpu:{frameMs:null},gpu:{frameMs:null},memory:{gpuBytes:null},draw:{submitted:null},customMetrics:{failure}});
      return {timestamp,config,environment,scene:sceneInfo,quality:{passed,captures,checkedFrames,selectionOracle,...(failure?{failure}:{})},blocks,records,limitations,preparationMs,qualityMs:performance.now()-qualityStart};
    };
    for(const [captureIndex,index] of checkedFrames.entries()){emit({phase:'quality-control',message:`Vérification visuelle exacte : image ${index+1}/${config.samples}.`,completed:captureIndex,total:checkedFrames.length,unit:'captures'});await nextFrame();check();
      const a=capture(baseline,index),repeat=capture(baseline,index),b=capture(config.candidate,index);
      let repeatDifferentBytes=0,candidateDifferentBytes=0,maxRepeatChannelDifference=0,maxCandidateChannelDifference=0;
      for(let i=0;i<a.length;i++){
        if(a[i]!==repeat[i]){repeatDifferentBytes++;maxRepeatChannelDifference=Math.max(maxRepeatChannelDifference,Math.abs(a[i]-repeat[i]));}
        if(a[i]!==b[i]){candidateDifferentBytes++;maxCandidateChannelDifference=Math.max(maxCandidateChannelDifference,Math.abs(a[i]-b[i]));}
      }
      captures.push({index,baselineHash:await hash(a),repeatHash:await hash(repeat),candidateHash:await hash(b),repeatDifferentBytes,candidateDifferentBytes,
        maxChannelDifference:Math.max(maxRepeatChannelDifference,maxCandidateChannelDifference),maxRepeatChannelDifference,maxCandidateChannelDifference,
        candidateShadowPassTriangles:shadowTriangles,cachedShadowChecked:shadowMapReused});
      emit({phase:'quality-control',message:`Contrôle visuel ${captureIndex+1}/${checkedFrames.length} terminé.`,completed:captureIndex+1,total:checkedFrames.length,unit:'captures'});
    }
    if(isAdaptive(config.candidate)){
      emit({phase:'quality-control',message:'Oracle de visibilité sur tout le parcours, hors chronométrage.',completed:0,total:config.samples,unit:'frames'});
      setMode(config.candidate);
      coherence.havePreviousPlanes=false;
      for(const entry of sphereEntries)resetAdaptiveEntry(entry);
      for(let index=0;index<config.samples;index++){
        await nextFrame();check();pose(index);cull();
        selectionOracle.sampleCount++;
        for(const entry of sphereEntries){
          if((entry.mesh.layers.mask!==0)!==frustum.intersectsObject(entry.mesh)){
            selectionOracle.mismatches++;
            selectionMismatches++;
          }
        }
      }
        selectionOracle.checked=true;
    }
    ...
    for(const [blockIndex,variant] of sequence.entries()){
      check();setMode(variant);emit({phase:'block-warmup',message:`Bloc ${blockIndex+1}/4 : ${variant}, échauffement puis ${config.samples} images.`,completed:0,total:config.warmup,unit:'frames'});
      ...
      for(let i=0;i<config.warmup;i++){
        ...
        if(i%30===29||i===config.warmup-1)emit({phase:'block-warmup',message:`Bloc ${blockIndex+1}/4 : échauffement ${i+1}/${config.warmup}.`,completed:i+1,total:config.warmup,unit:'frames'});
      }
      ...
      for(let index=0;index<config.samples;index++){
        ...
        if(index%30===29||index===config.samples-1)emit({phase:'measurement',message:`Bloc ${blockIndex+1}/4 : ${index+1}/${config.samples} images mesurées.`,completed:index+1,total:config.samples,unit:'frames'});
      }
      blocks.push(block);
    }
    emit({phase:'gpu-drain',message:'Collecte non bloquante des dernières durées GPU.',completed:0,total:queries.length,unit:'frames'});
    for(let i=0;i<120&&queries.length;i++){await nextFrame();check();poll();if(i%15===14||!queries.length)emit({phase:'gpu-drain',message:`Collecte GPU : ${queries.length} requête(s) restante(s).`,completed:i+1,total:120,unit:'frames'});}
    ...
    const result=report(true);result.qualityMs=qualityMs;result.environment.gpuDisjointEvents=gpuDisjointEvents;result.environment.gpuUnresolvedQueries=queries.length;
    emit({phase:'complete',message:'Mesures terminées ; résultat prêt à archiver.',completed:4,total:4,unit:'blocks'});return result;
  } catch(error) {
    if(signal?.aborted)onLifecycle?.({phase:'cancelled',message:'Exécution annulée ; dernière image valide conservée.'});
    throw error;
  } finally {if(!ownsPreview)cleanup();}
}
export async function runWorldComparison(canvas:HTMLCanvasElement,options:WorldOptions,onProgress:(message:string)=>void=()=>{},signal?:AbortSignal,onMetrics?:(metrics:WorldPreviewMetrics)=>void,onLifecycle?:(progress:WorldProgress)=>void):Promise<WorldReport>{
  return await execute(canvas,options,onProgress,signal,undefined,undefined,onMetrics,onLifecycle) as WorldReport;
}
export async function startWorldPreview(canvas:HTMLCanvasElement,options:WorldOptions,onFrame:(metrics:WorldPreviewMetrics)=>void=()=>{},signal?:AbortSignal,onError?:(error:Error)=>void,onComplete?:(completion:WorldPreviewCompletion)=>void,onLifecycle?:(progress:WorldProgress)=>void):Promise<()=>void>{
  return await execute(canvas,options,()=>{},signal,onFrame,onError,undefined,onLifecycle,onComplete) as ()=>void;
}
