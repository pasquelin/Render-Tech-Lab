import { runWorldComparison, startWorldPreview } from './worldScene.ts';
import type { WorldOptions, WorldPreviewMetrics } from './worldTypes.ts';
import { formatWorldReport, worldRows, modeLabels, ms, type SavedWorldReport } from './worldReporter.ts';
import { createWorldMatrix, worldMatrixTotals, type WorldMatrixAxes } from './worldMatrix.ts';

interface Archive { timestamp: string; config: WorldOptions; jsonUrl: string; markdownUrl: string; sourcesUrl?: string | null }
...
export function mountWorldWorkbench(): void {
  ...
  const status = (value: string) => { text('bench-status', value); };
  ...
  for (const anchor of el('lab-run-card').querySelectorAll<HTMLAnchorElement>('a[href="/?test=14-open-world"]')) anchor.classList.add('hidden');
  ...
  countSelect.replaceChildren(...[1, 9, 25].map(value => new Option(`${value} quartier${value > 1 ? 's' : ''}`, String(value), false, value === 9)));
  ...
  el('stat-fps').parentElement!.querySelector('.stat-title')!.textContent = 'FPS · rAF';
  ...
  function stop() { stopSystemMonitor(); active?.abort(); stopPreview?.(); stopPreview = null; }
  function freshCanvas() {
    const next = canvas.cloneNode(false) as HTMLCanvasElement; canvas.replaceWith(next); canvas = next;
    canvas.style.display = ''; canvas.style.objectFit = 'contain';
  }
  ...
  async function preview(candidate: boolean) {
    if (active || pending) return;
    selectPreviewButton(candidate);
    const controller = new AbortController(); active = controller; busy(true);
    status('Chargement du décor Bistro et préparation du rendu…');
    try {
      const config = options(); config.previewMode = candidate ? config.candidate : config.candidate === 'frustum' ? 'brute' : 'frustum';
      describe(config); freshCanvas(); text('stat-mode', modeLabels[config.previewMode]);
      let previewFailure: Error | null = null;
      const cleanup = await startWorldPreview(canvas, config, metricHandler(false), controller.signal, error => { previewFailure = error; controller.abort(); });
      stopPreview = cleanup;
      if (!controller.signal.aborted) {
        status('Aperçu actif. Les valeurs affichées ne constituent pas une campagne archivée.');
        await new Promise<void>(resolve => controller.signal.addEventListener('abort', () => resolve(), { once: true }));
      }
      cleanup(); status(previewFailure ? `Aperçu interrompu : ${String(previewFailure)}` : 'Aperçu arrêté. Derniers relevés conservés.');
    } catch (error) { status(controller.signal.aborted ? 'Aperçu arrêté.' : String(error)); }
    finally { active = null; stopPreview = null; busy(false); }
  }
  ...
  async function refreshComparisonChoices() {
    try {
      ...
      if (!listen.signal.aborted) updateComparisonChoices(runs);
  ...
  function reportStatistics(report: SavedWorldReport) {
    ...
    return { cpu: mean(cpu), rafP95: quantile(raf, .95), rafP99: quantile(raf, .99), cadence: interval !== null && interval > 0 ? 1000 / interval : null,
      gpu: mean(gpu), cpuSamples: cpu.length, rafSamples: raf.length, gpuSamples: gpu.length, frames: samples.length };
  }
  function renderComparison() {
    ...
    row('CPU moyen', (side) => `${measuredDuration(statistics[side]!.cpu)} · ${statistics[side]!.cpuSamples}/${statistics[side]!.frames} images`);
    ...
    row('GPU moyen', (side) => statistics[side]!.gpu === null ? 'Non mesuré' : `${duration(statistics[side]!.gpu)} · ${statistics[side]!.gpuSamples}/${statistics[side]!.frames} images`);
  ...
  async function save() {
    if (!pending) return;
    const response = await fetch('/api/world-comparison', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(pending) });
  ...
  async function run() {
    ...
    try {
      ...
      for (const config of planned) {
        ...
        const afterResponse = await fetch('/api/world-comparison/meta');
        ...
        if (!sourcesStable) throw new Error('Sources modifiées pendant le test : rapport conservé, mesures non recevables.');
    ...
    } catch (error) { status(controller.signal.aborted ? 'Campagne arrêtée.' : String(error)); retryButton.classList.toggle('hidden', pending === null); }
  ...
  on(moduleSelect, 'change', () => { stop(); window.location.assign(`/?test=${encodeURIComponent(moduleSelect.value)}`); });
  on(controls, 'change', updatePlan); on(modeControls, 'change', updatePlan); on(countSelect, 'change', updatePlan);
  on(referenceButton, 'click', () => { void preview(false); }); on(candidateButton, 'click', () => { void preview(true); });
