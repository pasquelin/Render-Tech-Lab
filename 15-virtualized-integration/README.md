...
Les vues texturée et filaire utilisent le moteur A choisi. `webgpu-page-raster` consomme `createGpuPageCache` avec la même coupe CPU `lodScore` ; s’il est absent ou perdu, l’explorateur reste sur le chemin Three.js sans notice utilisateur. Les diagnostics clusters et pages utilisent les feuilles exactes (ou le raster WebGPU s’il est le moteur A) ; la vue pages colore les pages d’indices effectivement attachées. Les transparences partagées conservent leurs matériaux. GPU-driven selection et visibility buffer ne sont pas branchés. Un dépassement du budget de pages interrompt le diagnostic au lieu de dessiner une surface incomplète.

Le parcours v1 rejoue huit segments géométriques : 60 poses par segment après 30 images de préchauffage. Il est déterministe et exportable ; les rues et zones de végétation ne sont pas encore calibrées. Les rapports conservent les poses, les échantillons réels, la résolution, la clé du cache, les erreurs/replis et les captures de repérage. Les captures peuvent affecter l’intervalle rAF suivant et ne prouvent aucune fidélité A/B. CPU et rAF ont des distributions séparées ; GPU et VRAM restent non mesurés.
