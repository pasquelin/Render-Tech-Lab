...
import {playerHeightFor} from '../implementation/navigation.ts';
...
/** Writes `navigation.bin` next to the prepared cache of one catalogue model. */
export async function prepareNavigationMesh(modelId: string): Promise<void> {
  ...
  const height = playerHeightFor(Math.min(size.x, size.z));
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url))
  await prepareNavigationMesh(process.argv[2] ?? 'emerald-square');
