...
for (const model of selected) {
  ...
  const result = await prepare(sourceDirectory, derivedDirectory, 'full', 150000, {
    ...
    onProgress: event => { if (event.phase === 'ready') console.log(JSON.stringify({ model: model.id, ...event })); },
  });
  if (result.status !== 'ready') throw new Error(`${model.id} : préparation incomplète (${result.status})`);
