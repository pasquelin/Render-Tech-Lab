...
test('UI archive retains partial errors in a unique file and writes the common report',async()=>{
 ...
 try{await mkdir(join(root,'15-virtualized-integration','implementation'),{recursive:true}); await mkdir(join(root,'15-virtualized-integration','runner')); 
  ...
  const saved=JSON.parse(gunzipSync(await readFile(join(root,a.artifact))).toString('utf8'));assert.deepEqual(saved.result,result);assert.ok(saved.sources['clusterGpu.ts'].sha256);
  assert.match(await readFile(join(root,'reports','15-virtualized-integration',`campaign-${a.id}`,'REPORT.md'),'utf8'),/cancelled/);
