...
En exploration manuelle, la caméra et la ville de départ se règlent dans la colonne de droite, scène ouverte. « Libre à pied » garde l’horizon horizontal : WASD et flèches déplacent à hauteur stable, R/F changent l’altitude, la souris glissée regarde autour. « Jeu à la première personne » capture la souris après un clic sur le canvas ; WASD et flèches déplacent, Espace saute, Maj accélère, Échap libère la souris. Un départ praticable est recherché sur le sol de la copie du modèle choisie parmi 1, 4, 9 ou 12 villes. Le POC de collision lit un fichier de triangles extrait des géométries du `source.gltf` préparé ; les surfaces transparentes décoratives sont écartées, mais les vitrages nommés sont conservés. Il indexe ces triangles dans une grille spatiale, les partage entre les copies de la ville et calcule les contacts du joueur ainsi que la hauteur du sol sur leurs surfaces. Les bornes servent uniquement à rechercher les triangles proches, jamais à bloquer le joueur. Cette géométrie est simplifiée et quantifiée pour rester exploitable en navigation ; les détails écartés peuvent donc manquer aux collisions. `prepare:models` génère également le fichier de navigation pour chaque cache préparé. Le mode jeu reste une exploration diagnostique et ses échantillons ne sont pas des mesures comparatives officielles.
